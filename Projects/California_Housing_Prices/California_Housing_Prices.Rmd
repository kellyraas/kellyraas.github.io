...
First we need to make sure that the data looks how we expect it and that no datapoint is missing.
...
There is no missing data which is good. Also the rest of the data looks ok and we do not need to change any variable types.
...
Among the variable total_rooms, total_bedrooms, population and households there is also a high correlation, which could cause a multicollinearity problem. However, in the model these variables are changed 

## Data Cleaning and Preprocessing

### Create model vriables

Given the following model we want to fit, we need to create follwing new variables
...
and drop the variables that we will not need for the model.
...
It is good practice to normalize the data before training a neural network. We will only scale the independent variables, not our dependent variable 'median_house_value' since to be able to later compare the performance of different models it is better to have unnormalized predictions. We will use the min-max method and scale the data in the interval [0,1]. 
...
Given that we want to fit a model that makes predictions, one way of evaluating it is by calculating the MSE (mean square error), which is the average squared difference between the predicted values and the actual ones. Hence, MSE is a measure of how far our predictions are away from the acual data.
...
There is no fixed rule as to how many layers and neurons to use when modeling a neural net, aalthough there are several more or less accepted rules of thumb. Usually finding the best fit is a matter of trial and error. From the problem’s objective, we know that the structure of the ANN will have 9 inputs and 1 output that will be the prediction. As to the hidden layers different possibilities will be considered. 
...
In order for the model to converge I had to raise the defoult threshold value from 0.01 to 0.1, now we can visualize the net.
...
Thus, we can Check the model's performance on the test data and calculate the MSE. 
...
So we will stick with our first model for now and see what happens if we alter the activation function from the default logistic to a tangent hyperbolicus.
...
Let's now see what happens if we allow for five repetitions (or epochs).
